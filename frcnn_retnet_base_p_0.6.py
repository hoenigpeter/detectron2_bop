...
seq = iaa.Sequential([
    Sometimes(p_augmentation * 0.5, CoarseDropout( p=0.2, size_percent=0.05) ),
    Sometimes(p_augmentation * 0.4, GaussianBlur((0., 3.))),
    Sometimes(p_augmentation * 0.3, pillike.EnhanceSharpness(factor=(0., 50.))),
    Sometimes(p_augmentation * 0.3, pillike.EnhanceContrast(factor=(0.2, 50.))),
    Sometimes(p_augmentation * 0.5, pillike.EnhanceBrightness(factor=(0.1, 6.))),
    Sometimes(p_augmentation * 0.3, pillike.EnhanceColor(factor=(0., 20.))),
    Sometimes(p_augmentation * 0.5, Add((-25, 25), per_channel=0.3)),
    Sometimes(p_augmentation * 0.3, Invert(0.2, per_channel=True)),
    Sometimes(p_augmentation * 0.5, Multiply((0.6, 1.4), per_channel=0.5)),
    Sometimes(p_augmentation * 0.5, Multiply((0.6, 1.4))),
    Sometimes(p_augmentation * 0.1, AdditiveGaussianNoise(scale=10, per_channel=True)),
    Sometimes(p_augmentation * 0.5, iaa.contrast.LinearContrast((0.5, 2.2), per_channel=0.3)),
    ], random_order=True)
...
def main(args):
    ...
    output_dir = "/hdd/detectron2_models_new/" + args.model_type + "_" + dataset_info["dataset"] + "_model_p_0.6"
