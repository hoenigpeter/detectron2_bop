...
def train(dataset="tless", texture="", num_classes=30):
    ...
    MetadataCatalog.get(dataset + texture + "_pbr_train").thing_classes = MetadataCatalog.get(dataset + "_pbr_meta").thing_classes
    MetadataCatalog.get(dataset + texture + "_pbr_train").thing_dataset_id_to_contiguous_id = MetadataCatalog.get(dataset + "_pbr_meta").thing_dataset_id_to_contiguous_id

    print(MetadataCatalog.get(dataset + "_pbr_meta"))
...
def test(dataset="tless", texture="", num_classes=30):
    ...
    dataset_temp = DatasetCatalog.get(dataset + "_meta")
    print(MetadataCatalog.get(dataset + "_meta"))
    metadata_temp = MetadataCatalog.get(dataset + "_meta")

    def bop_scenes_to_coco():
        scene_ids = [*range(1, 21, 1)]
        scene_list = []
        print(scene_ids)
        
        for id in scene_ids:
            scene_list.extend(load_coco_json("./datasets/BOP_DATASETS/" + dataset + "/test/{:06d}/scene_gt_coco.json".format(id), "./datasets/BOP_DATASETS/" + dataset + "/test/{:06d}".format(id), dataset + "_meta", None))
        return scene_list

    dataset_name = dataset + "_bop_test"
    print(dataset_name)
    DatasetCatalog.register(dataset_name, bop_scenes_to_coco)

    print("dataset catalog: ", DatasetCatalog.list())
    metadata = MetadataCatalog.get(dataset_name)

    metadata.set(thing_classes = metadata_temp.thing_classes)
    metadata.set(thing_dataset_id_to_contiguous_id = metadata_temp.thing_dataset_id_to_contiguous_id)
    ...
    cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST = 0.1  # Set the desired threshold for detection
    cfg.MODEL.ROI_HEADS.NUM_CLASSES = num_classes  # Adjust according to your dataset
