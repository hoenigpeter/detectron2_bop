...
SPLITS_ITODD_VAL = dict(
    itodd_bop_val=dict(
        ...
        ann_file=osp.join(DATASETS_ROOT, "BOP_DATASETS/itodd/val_targets_bop19.json"),
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_ITODD_VAL:
        used_cfg = SPLITS_ITODD_VAL[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, ITODD_BOP_VAL_Dataset(used_cfg))
...
def get_available_datasets():
    return list(SPLITS_ITODD_VAL.keys())
